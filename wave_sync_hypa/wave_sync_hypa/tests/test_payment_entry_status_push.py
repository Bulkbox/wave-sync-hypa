"""Unit tests for the Payment Entry status-push pipeline.
...
Two layers:
  ...
  2. The on_submit hook (`on_payment_entry_submit`) — per Wave order asks
     payment_status_resolver for COMPLETED vs PAYMENT_PENDING and dispatches
     with forced_payload.

The resolver and dispatcher are patched at the module boundary so each test
focuses on the PE handler's branching.
"""
...
class TestStampWaveOrderId(FrappeTestCase):
	...
	def test_no_op_for_journal_entry_only_refs(self):
		...
		# Non-(SI/SO) refs are skipped before any DB call.
		mock_get_value.assert_not_called()
		mock_log.assert_not_called()


class TestOnPaymentEntrySubmit(FrappeTestCase):
	"""on_submit: per Wave order, compute status and dispatch with forced_payload."""
	...
	def test_dispatches_completed_for_fully_paid_wave_order(self):
		doc = _pe(references=[_ref("Sales Invoice", "SI-001")])
		with (
			patch.object(frappe.db, "get_value", return_value=WAVE_ID_A),
			patch.object(
	...
	def test_dispatches_payment_pending_for_partial_settlement(self):
		doc = _pe(references=[_ref("Sales Invoice", "SI-001")])
		with (
			patch.object(frappe.db, "get_value", return_value=WAVE_ID_A),
			patch.object(
				payment_status_resolver,
				"resolve_status_for_wave_order",
				return_value=payment_status_resolver.STATUS_PAYMENT_PENDING,
			),
			patch.object(order_status, "dispatch_with_wave_order_ids") as mock_dispatch,
		):
			pe_handler.on_payment_entry_submit(doc)
		mock_dispatch.assert_called_once()
		_, kwargs = mock_dispatch.call_args
		self.assertEqual(kwargs["forced_payload"], {"status": "PAYMENT_PENDING"})

	def test_per_wave_order_dispatch_with_distinct_payloads(self):
		"""One PE settling two Wave orders, one full + one partial -> two dispatches with distinct payloads."""
		...
		def _resolve(_pe_doc, wave_order_id):
			return (
				payment_status_resolver.STATUS_COMPLETED if wave_order_id == WAVE_ID_A
				else payment_status_resolver.STATUS_PAYMENT_PENDING
			)
		...
		payloads = [
			(call.args[2][0], call.kwargs["forced_payload"])
			for call in mock_dispatch.call_args_list
		]
		self.assertIn((WAVE_ID_A, {"status": "COMPLETED"}), payloads)
		self.assertIn((WAVE_ID_B, {"status": "PAYMENT_PENDING"}), payloads)

	def test_no_op_when_no_wave_orders_reachable(self):
		"""PE with non-Wave refs: no dispatch, no resolver call."""
		doc = _pe(references=[_ref("Journal Entry", "JE-001")])
		with (
	...
	def test_falls_back_to_stamped_wave_order_id_when_refs_have_none(self):
		"""References without wave_order_ids but doc has one stamped -> dispatch with that id."""
